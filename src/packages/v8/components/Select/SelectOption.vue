...
const props = withDefaults(
  defineProps<{
    ...
    /** 多選模式:已選項顯示勾選 */
    multiple?: boolean;
    /** 虛擬化 a11y:清單總數(aria-setsize) */
    setsize?: number;
    /** 虛擬化 a11y:本項位置,1-based(aria-posinset) */
    posinset?: number;
  }>(),
  { matched: () => [], multiple: false }
);
...
<template>
  <li
    ...
    :class="{
      ...
      'bs-select__option--multi': multiple,
    }"
  ...
  >
    <span class="bs-select__option-label">
      <slot>
    ...
    <span v-if="multiple && selected" class="bs-select__check" aria-hidden="true"
      >✓</span
    >
  </li>
</template>
